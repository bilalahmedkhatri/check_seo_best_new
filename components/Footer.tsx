...
const NewsletterForm: React.FC = () => {
      ...
      <div className="bg-gradient-to-r from-[#667eea] to-[#764ba2] p-8 rounded-2xl">
        ...
        <div className="mt-4 text-sm text-purple-200">
          <label className="flex items-center cursor-pointer">
            ...
            />
            <span className="ml-2">I agree to the <a href="#privacy" className="underline hover:text-white">Privacy Policy</a>.</span>
          </label>
        </div>
        {message && (
          <p className={`mt-4 text-center font-bold transition-opacity duration-300 ${status === 'idle' ? 'opacity-0' : 'opacity-100'} ${status === 'success' ? 'text-green-300' : 'text-red-300'}`}>
            {message}
          </p>
        )}
      </div>
  ...
  const FooterColumn: React.FC<{ title: string; links: { name: string; href: string; badge?: string }[]; }> = ({ title, links }) => {
    ...
    return (
      <div>
        ...
        >
          {title}
          <span className={`md:hidden transform transition-transform duration-300 ${isOpen ? 'rotate-180' : ''}`}>▼</span>
        </button>
  ...
  const CookieBanner: React.FC<{ isVisible: boolean; onDecision: (decision: 'accepted' | 'declined') => void; }> = ({ isVisible, onDecision }) => {
      ...
      <div className="fixed bottom-0 left-0 right-0 bg-[#2d2d2d] text-white p-6 shadow-2xl z-[100] animate-slide-up">
        <div className="w-full max-w-6xl mx-auto flex flex-col md:flex-row items-center justify-between gap-6">
          <div className="text-center md:text-left">
            ...
            <p className="text-gray-300 text-sm mt-1">We use cookies to enhance your browsing experience and analyze our traffic. By clicking "Accept All", you consent to our use of cookies.</p>
          </div>
          <div className="flex-shrink-0 flex items-center gap-3">
            ...
            <button onClick={() => alert('Detailed cookie preferences would be shown here.')} className="px-4 py-2 rounded-lg hover:bg-gray-700 transition font-semibold text-sm">Customize</button>
    ...
    return (
      <>
        <button
          ...
          className="fixed bottom-6 right-6 w-16 h-16 rounded-full bg-gradient-to-r from-[#667eea] to-[#764ba2] text-white shadow-lg flex items-center justify-center animate-pulse-glow hover:scale-110 transition-transform z-50 text-2xl"
        >
          💬
        </button>
        ...
        <button
          ...
          className={`group fixed bottom-28 right-8 w-12 h-12 rounded-full bg-gray-700 text-white shadow-lg flex items-center justify-center z-50 transition-all duration-300 ${isScrollTopVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-4 pointer-events-none'}`}
        >
...
const Footer: React.FC = () => {
  ...
  const footerData = {
    product: [ { name: "Features", href: "#" }, { name: "Pricing", href: "#" }, { name: "Integrations", href: "#" }, { name: "API Documentation", href: "#" }, { name: "Changelog", href: "#" }, { name: "Request Demo", href: "#" } ],
      ...
      <footer ref={footerRef} className="bg-[#1a1a1a] text-gray-300 pt-20 pb-8 mt-auto">
        <div className="w-full max-w-7xl mx-auto px-6 lg:px-8">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-6 gap-12">
            <div className="lg:col-span-2 md:col-span-2 animate-on-scroll fade-in">
            ...
            <div className="animate-on-scroll fade-in delay-500 md:col-start-1 lg:col-start-auto"><FooterColumn title="Company" links={footerData.company} /></div>
          </div>
          ...
          <div className="flex flex-col md:flex-row justify-between items-center gap-8 animate-on-scroll fade-in">
            <div className="flex items-center space-x-4">
              {Object.entries(socialLinksData).map(([platform, path]) => (
          ...
          <div className="text-center text-sm animate-on-scroll fade-in">
            <div className="flex flex-wrap justify-center gap-x-6 gap-y-2 mb-4">
              {["Privacy Policy", "Terms of Service", "Cookie Policy", "Sitemap"].map(link => (
                 <a key={link} href={`#${link.toLowerCase().replace(' ','-')}`} className="text-gray-400 hover:text-white transition">{link}</a>
              ))}
              <button onClick={() => setIsCookieBannerVisible(true)} className="text-gray-400 hover:text-white transition underline">Cookie Settings</button>
            </div>
            <p className="text-gray-500">&copy; {new Date().getFullYear()} SEO Master, Inc. &bull; Made with ❤️ for SEO professionals.</p>
          </div>
        </div>
      </footer>
      <CookieBanner isVisible={isCookieBannerVisible} onDecision={handleCookieDecision} />
      <FloatingButtons />
    </>
  );
};
